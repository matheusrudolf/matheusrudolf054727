...
import { <%= underscore(name).toUpperCase() %>_IMPORTS, NESTED_IMPORTS } from './<%= dasherize(name) %>.imports';

@Component({
    selector: 'app-<%= dasherize(name) %>',
    standalone: true,
    // 2. USO DA CONSTANTE NO ARRAY 'imports'
    imports: [<%= underscore(name).toUpperCase() %>_IMPORTS, NESTED_IMPORTS],
    template: `
        <p-table #dt [value]="page.dados" [columns]="columns" [resizableColumns]="true" [loading]="loading"
            ...
            <ng-template #caption>
                <app-<%= dasherize(name) %>-toolbar [datasource]="page.dados" [columns]="columns" [selectedColumns]="selectedColumns"
        ...
        @if (formProperties.visible) {
            <app-<%= dasherize(name) %>-form [visible]="formProperties.visible" [state]="formProperties.state"
...
export class <%= classify(name) %>Component implements OnInit {
    ...
    public page: Pageable<<%= classify(name) %>> = new Pageable<<%= classify(name) %>>();
    public columns: DatagridColumnType[] = [];
    public selectedRows: <%= classify(name) %>[] = [];
    public loading: boolean = false;
    public selectedColumns: DatagridColumnType[] = [];
    public formProperties: FormStateType<<%= classify(name) %>> = { visible: false, state: CrudStateEnum.none, formData: null };
    ...
    public handleGetRequest(filter?: any, sort?: any): void {
        ...
        this.loading = true;
        this.<%= dasherize(name) %>Service.listPageable(params, this.page)
    ...
    public handleDeleteEntity(rowData: <%= classify(name) %>): void {
        this.dialogUtil.confirmRemoveDialog(() => {
            this.<%= dasherize(name) %>Service.delete(rowData.id)
                .pipe(finalize(() => this.loading = false))
                .subscribe({
                    next: (res) => {
