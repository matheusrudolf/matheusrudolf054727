...
import { PERFIS_TOOLBAR_IMPORTS } from './perfis-toolbar.imports';

@Component({
    selector: 'app-perfis-toolbar',
    standalone: true,
    // 2. USO DA CONSTANTE NO ARRAY 'imports'
    imports: [PERFIS_TOOLBAR_IMPORTS],
    template: `
...
export class PerfisToolbarComponent implements OnChanges {
    ...
    private handleBuildExportMenu(): void {
        this.exportMenu = [
            { icon: 'pi pi-file-excel', label: 'Exportar dados em XLSX', command: () => this.exportService.exportExcel(this.datasource, 'perfis') },
            { icon: 'pi pi-file-excel', label: 'Exportar dados selecionados em XLSX', disabled: this.selectedRows.length === 0, command: () => this.exportService.exportExcel(this.selectedRows, 'perfis') },
            { separator: true },
            { icon: 'pi pi-file-pdf', label: 'Exportar dados em PDF', command: () => this.exportService.exportPdf(this.columns, this.datasource, 'perfis') },
            { icon: 'pi pi-file-pdf', label: 'Exportar dados selecionados em PDF', disabled: this.selectedRows.length === 0, command: () => this.exportService.exportPdf(this.columns, this.selectedRows, 'perfis') },
        ];
    }
    ...
    public handleDeleteSelectedRows(): void {
        ...
        this.dialogUtil.confirmRemoveAllDialog(() => {
            this.perfisService.deleteSelected(ids).subscribe({
                next: (res) => {
